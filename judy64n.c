...
//	judy_cell:	insert a string into the judy array, return cell
//pointer. 	judy_strt:	retrieve the cell pointer greater than or equal to given
//key 	judy_slot:	retrieve the cell pointer, or return NULL for a given
//key. 	judy_key:	retrieve the string value for the most recent judy
//query. 	judy_end:	retrieve the cell pointer for the last string in the
//array. 	judy_nxt:	retrieve the cell pointer for the next string in the
//array. 	judy_prv:	retrieve the cell pointer for the prev string in the
//array. 	judy_del:	delete the key and cell for the current stack entry.
...
#define PRIuint "u"
    ...
    defined(_WIN64) || defined(__sparc64__) || defined(__arch64__) ||          \
...
int JudySize[] = {(JUDY_slot_size * 16),            // JUDY_radix node size
  ...
  while (next) {
    ...
    case JUDY_64:
      ...
      if (test == value) {
        ...
        if (!judy->depth && !(value & 0xFF) ||
            judy->depth && depth == judy->depth)
          return &node[-slot - 1];
    ...
    case JUDY_radix:
      ...
      if (judy->depth)
        slot = (src[depth] >> ((JUDY_key_size - off++ & JUDY_key_mask) * 8)) &
               0xff;
      else if (off < max)
        slot = buff[off++];
      else
...
JudySlot *judy_promote(Judy *judy, JudySlot *next, int idx, judyvalue value,
  ...
  *next = (JudySlot)newbase | type;
  ...
  memcpy(newbase + (newcnt - oldcnt - 1) * keysize, base,
         idx * keysize); // copy keys
...
#if BYTE_ORDER != BIG_ENDIAN
  memcpy(newbase + (idx + newcnt - oldcnt - 1) * keysize, &value,
         keysize); // copy key
#else
  i = keysize;
  ...
  memcpy(newbase + (idx + newcnt - oldcnt) * keysize, base + (idx * keysize),
         (oldcnt - slot) * keysize); // copy keys
...
void judy_radix(Judy *judy, JudySlot *radix, uchar *old, int start, int slot,
                int keysize, uchar key, uint depth) {
  ...
  if (!judy->depth && (!key || !keysize) ||
      judy->depth && !keysize && depth == judy->depth) {
...
#if BYTE_ORDER != BIG_ENDIAN
    memcpy(base + (newcnt - idx - 1) * keysize,
           old + (start + cnt - idx - 1) * (keysize + 1), keysize);
#else
    memcpy(base + (newcnt - idx - 1) * keysize,
           old + (start + cnt - idx - 1) * (keysize + 1) + 1, keysize);
#endif
...
JudySlot *judy_first(Judy *judy, JudySlot next, uint off, uint depth) {
  ...
  while (next) {
    ...
    case JUDY_64:
      ...
      judy->stack[judy->level].slot = slot;
#if BYTE_ORDER != BIG_ENDIAN
      if (!judy->depth && !base[slot * keysize] ||
          judy->depth && ++depth == judy->depth)
        return &node[-slot - 1];
#else
      if (!judy->depth && !base[slot * keysize + keysize - 1] ||
          judy->depth && ++depth == judy->depth)
        return &node[-slot - 1];
#endif
      next = node[-slot - 1];
...
#if BYTE_ORDER != BIG_ENDIAN
      if (!judy->depth && !base[(slot - 1) * keysize] ||
          judy->depth && ++depth == judy->depth)
#else
      if (!judy->depth && !base[(slot - 1) * keysize + keysize - 1] ||
          judy->depth && ++depth == judy->depth)
#endif
        return &node[-slot];
...
JudySlot *judy_cell(Judy *judy, uchar *buff, uint max) {
  ...
  while (*next) {
    ...
    default:
      ...
      if (!node[-1]) {
        memmove(base, base + keysize,
                slot * keysize); // move keys less than new key down one slot
#if BYTE_ORDER != BIG_ENDIAN
        memcpy(base + slot * keysize, &value,
               keysize); // copy new key into slot
#else
        test = value;
        idx = keysize;
    ...
    case JUDY_radix:
      ...
      if (!judy->depth && !slot ||
          judy->depth && depth == judy->depth) { // leaf?
#ifdef ASKITIS
        if (*next)
          Found++;
        else
...
//	naskitis.com.
//	g++ -O3 -fpermissive -fomit-frame-pointer -w -D STANDALONE -D ASKITIS -o
//judy64n judy64n.c
//	./judy64n [input-file-to-build-judy] e.g. distinct_1 or skew1_1

//	note:  the judy array is an in-memory data structure. As such, make sure
//you 	have enough memory to hold the entire input file + data structure,
//otherwise 	you'll have to break the input file into smaller pieces and load
//them in 	on-by-one.
...
int main(int argc, char **argv) {
  ...
  for (off = 0; off < size; off++)
    if (askitis[off] == '\n') {
      *(judy_cell(judy, askitis + prev, off - prev)) +=
          1; // count instances of string
      prev = off + 1;
    }
    //	naskitis.com:
    //	Stop the timer and do some math to compute the time required to insert
    //the strings into the judy array.

#if !defined(_WIN32)
  gettimeofday(&stop, NULL);

  insert_real_time = 1000.0 * (stop.tv_sec - start.tv_sec) +
  ...
  //	naskitis.com:
  //	Free the input buffer used to store the first file.  We must do this
  //before we get the process size below.
  free(askitis);
  fprintf(stderr,
          "JudyArray@Karl_Malbrain\nDASKITIS option "
          "enabled\n-------------------------------\n%-20s %.2f MB\n%-20s %.2f "
          "sec\n",
          "Judy Array size:", MaxMem / 1000000.,
          "Time to insert:", insert_real_time);
  ...
  for (off = 0; off < size; off++)
    ...
    //	naskitis.com:
    //	Stop the timer and do some math to compute the time required to search
    //the judy array.

#if !defined(_WIN32)
  gettimeofday(&stop, NULL);
  search_real_time = 1000.0 * (stop.tv_sec - start.tv_sec) +
                     0.001 * (stop.tv_usec - start.tv_usec);
